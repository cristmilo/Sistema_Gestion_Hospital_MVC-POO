...
class MedicoView(BaseView):
    ...
    def _construir_ui(self):
        ...
        self.tabla = self.crear_tabla(tabla_frame, ("ID","Nombre","Especialidad","Email"))
        ...
        tk.Button(exp, text="📊 Excel", bg="#388E3C", fg="white",
                  command=lambda: exportar_excel("Médicos", ["ID","Nombre","Especialidad","Email"],
                  [self.tabla.item(r)["values"] for r in self.tabla.get_children()])).pack(side="left", padx=6)
        tk.Button(exp, text="📄 PDF", bg="#D32F2F", fg="white",
                  command=lambda: exportar_pdf("Médicos", ["ID","Nombre","Especialidad","Email"],
                  [self.tabla.item(r)["values"] for r in self.tabla.get_children()])).pack(side="left", padx=6)
